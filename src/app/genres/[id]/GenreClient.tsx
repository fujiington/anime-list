...
import { browseAnime } from "@/lib/jikan";
...
export default function GenreClient({ genreId, genreName }: Props) {
  ...
  const fetchData = useCallback(async () => {
    const key = `${genreId}|${page}|${orderBy}|${sort}|${status}|${type}`;
  ...
  }, [genreId, page, orderBy, sort, status, type]);
  ...
  return (
    <div className="space-y-5">
      ...
      <SortBar orderBy={orderBy} sort={sort} status={status} type={type} onChange={handleFilter} />
